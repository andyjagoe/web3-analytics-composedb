...
const createWeb3AnalyticsComposite = async () => {
    const composite = await createComposite(ceramic, 'schema/event.graphql')
    await writeEncodedComposite(composite, 'src/__generated__/event.json')
}

export { createWeb3AnalyticsComposite }